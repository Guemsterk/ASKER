...
use AskerBundle\Entity\ResourceMetadataFactory;
...
class MetadataByResourceController extends BaseController
{
    ...
    {
        try {
            $metadatas = $this->get('asker.exercise.resource_metadata')->getAll(
    ...
    {
        try {
            $metadata = $this->get('asker.exercise.resource_metadata')->getByEntity(
                $resourceId,
                $metadataKey,
                $this->getUserId()
            );
    ...
    )
    {
        try {
            ...
            $metadata = $this->get('asker.exercise.resource_metadata')->addToEntity(
                $resourceId,
                $metadata,
                $this->getUserId()
            );
    ...
    )
    {
        try {
            ...
            $metadata = $this->get('asker.exercise.resource_metadata')
                ->saveFromEntity(
                    $resourceId,
                    $metadata,
                    $metadataKey,
    ...
    {
        try {
            $this->get('asker.exercise.resource_metadata')->removeFromEntity(
                $resourceId,
                $metadataKey,
                $this->getUserId()
            );
    ...
    {
        try {
            $resources = $this->get('asker.exercise.exercise_resource')
                ->editMetadata
                (
                    $resourceId,
                    $metadatas,
