...
namespace AskerBundle\Controller\Api\ExerciseResource;
...
class ResourceController extends BaseController
{
    ...
    {
        try {
            $resources = $this->get('asker.exercise.exercise_resource')->getAll(
    ...
    )
    {
        try {
            ...
            $exerciseResource = $this->get('asker.exercise.exercise_resource')->createAndAdd
                (
                    $resourceResource,
                    $userId
                );

			//throw new \LogicException('Im debbugging forcefully');
    ...
    public function editAction(ResourceResource $resourceResource, $resourceId)
    {
        try {
            ...
            $resource = $this->get('asker.exercise.exercise_resource')->edit
                (
                    $resourceResource,
                    $this->getUserId()
                );
    ...
    {
        try {
            $this->get('asker.exercise.exercise_resource')->remove(
                $resourceId,
                $this->getUserId()
            );
    ...
    {
        try {
            $resource = $this->get('asker.exercise.exercise_resource')->subscribe(
                $this->getUserId(),
                $resourceId
            );

            $resourceResource = $this->get('asker.exercise.exercise_resource')
    ...
    public function duplicateAction($resourceId)
    {
        try {
            ...
            $resource = $this->get('asker.exercise.exercise_resource')->duplicate(
                $resourceId,
                $this->getUserId()
            );
    ...
    public function importAction($resourceId)
    {
        try {
            /** @var ExerciseResource $resource */
            $resource = $this->get('asker.exercise.exercise_resource')->import(
                $this->getUserId(),
                $resourceId
            );
