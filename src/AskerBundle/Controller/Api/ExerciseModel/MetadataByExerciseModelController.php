...
use AskerBundle\Entity\ExerciseModelMetadataFactory;
...
class MetadataByExerciseModelController extends BaseController
{
    ...
    {
        try {
            $metadatas = $this->get('asker.exercise.exercise_model_metadata')->getAll(
    ...
    {
        try {
            $metadata = $this->get('asker.exercise.exercise_model_metadata')
                ->getByEntity(
                    $exerciseModelId,
                    $metadataKey,
                    $this->getUserId()
    ...
    )
    {
        try {
            ...
            $metadata = $this->get(
                'asker.exercise.exercise_model_metadata'
            )->addToEntity(
                    $exerciseModelId,
                    $metadata,
                    $this->getUserId()
    ...
    {
        try {
            $this->get('asker.exercise.exercise_model_metadata')->removeFromEntity(
                $exerciseModelId,
                $metadataKey,
                $this->getUserId()
            );
    ...
    {
        try {
            $resources = $this->get('asker.exercise.exercise_model')
                ->editMetadata
                (
                    $exerciseModelId,
                    $metadatas,
